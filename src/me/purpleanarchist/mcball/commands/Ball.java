	...
	public boolean onCommand(CommandSender sender, Command cmd,
		...
		if (sender instanceof Player) {
			...
			if (commandLabel.equalsIgnoreCase("ball")) {
				if (args.length == 0) {
					Util.sendMessage(p,
							"Use /ball help for help joining a game!");
				...
				if (args.length == 1) {
					if (args[0].equalsIgnoreCase("join")) {
						...
						p.sendMessage("/ball redbasket");
						p.sendMessage("/ball bluebasket");
					...
					if (args[0].equalsIgnoreCase("info")) {
						...
						Util.sendMessage(p, "Players in MC Ball "
								+ ArenaManager.getManager().getArena(args[1])
										.getName() + ":");
						for (String s : ArenaManager.getManager()
								.getArena(args[1]).getPlayers()) {
							p.sendMessage("§a"
									+ s
									+ ":§b "
						...
						ArenaManager.getManager().setStart(args[1],
								p.getLocation());
						Util.sendMessage(p, "set the start to MC Ball "
								+ args[1]);
